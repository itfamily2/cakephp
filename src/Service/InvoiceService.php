...
class InvoiceService
{
    ...
    public function generateFromOrder(int $orderId, ?string $notes = null): Invoice
    {
        ...
        // Business Rule: Generate invoice only if Order is Completed or Accepted
        if (!in_array($order->status, ['Accepted', 'Processing', 'Completed'])) {
            throw new Exception("Invoices can only be generated for Accepted or Completed orders. Current status: {$order->status}");
        }
