<?php $this->assign('title', 'Edit Order ' . h($order->order_number)); ?>
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h4 class="fw-bold mb-1 text-dark"><i class="fa-solid fa-cash-register text-primary me-2"></i>Edit Order #<?= h($order->order_number) ?></h4>
        <p class="text-muted mb-0">Modifying an existing order.</p>
    </div>
</div>

<?php
// Disable editing if the order is past the pending state, unless admin.
$isLocked = in_array($order->status, ['Completed', 'Invoiced', 'Closed']);
?>
...
<?php if ($isLocked): ?>
    <div class="alert alert-warning d-flex align-items-center mb-4 shadow-sm" role="alert">
        ...
        <div>
            <strong>Order Locked</strong><br>
            This order is in a <em><?= h($order->status) ?></em> state and cannot be modified.
        </div>
    </div>
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            document.querySelectorAll('#pos-form input, #pos-form button, #pos-form select, #pos-form textarea').forEach(el => {
                if(el.id !== 'checkout-btn') el.disabled = true;
            });
...
<?= $this->Form->create($order, ['id' => 'pos-form', 'url' => ['action' => 'edit', $order->id]]) ?>
<?php 
        ...
        <div class="card shadow-sm border-0 h-100">
            ...
            <div class="card-body p-4 bg-light">
                ...
                <button type="submit" class="btn btn-primary btn-lg w-100 shadow-sm fw-bold" id="checkout-btn" disabled>
                    <i class="fa-solid fa-lock me-2"></i>Update Order
                </button>
            </div>
        </div>
    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let cart = [
        <?php if (!empty($order->order_items)): ?>
            ...
            {
                id: <?= $item->product_id ?>,
                name: <?= json_encode($item->hasValue('product') ? $item->product->name : 'Product #'.$item->product_id) ?>,
                price: <?= (float)$item->price ?>,
                qty: <?= (int)$item->quantity ?>,
                maxStock: <?= $item->hasValue('product') ? ($item->product->stock + $item->quantity) : $item->quantity ?>
            },
            <?php endforeach; ?>
        <?php endif; ?>
    ];
    ...
    posForm.addEventListener('submit', function(e) {
        ...
        .then(data => {
            if (data.success) {
                ...
                Swal.fire({
                    title: 'Order Updated!',
                    text: data.message || 'Order was updated successfully.',
                    icon: 'success',
                    showCancelButton: true,
                    confirmButtonText: '<i class="fa-solid fa-arrow-left me-2"></i>Back to Ledger',
                    ...
                    if (result.isConfirmed) {
                        ...
                        checkoutBtn.innerHTML = '<i class="fa-solid fa-save me-2"></i>Update Order';
                    }
                });
            } else {
                // Show error notification
                Swal.fire('Error', data.message || 'Error updating order.', 'error');
                checkoutBtn.disabled = false;
                checkoutBtn.innerHTML = '<i class="fa-solid fa-save me-2"></i>Update Order';
            }
        })
